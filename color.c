...
enum {
  RGB = 2, HEX = 4, HSV = 8, HSL = 16,
  /* clops */
  NEGATE = 128, CONVERSION = 256
};
...
void displaycolor(color_t color, int options);
...
{
  // NOTE: I see no reason of `OPTIONS`, as we have clops
  int COLOR_MODEL = RGB, OPTIONS = 0, CLOPS[MAXCLOPS], clp = 0;
  char *var1, *var2, *var3;
  ...
  /* parse options */
  int op;
  while ((op = getopt_long(argc, argv_ptr, "nt:", arg_options, NULL)) != -1) {
    switch (op) {
    case 'n':
      OPTIONS |= NEGATE;
      CLOPS[clp++] = NEGATE;
    ...
    case 't':
      OPTIONS |= CONVERSION;
      if (!strcmp(optarg, "hex")) OPTIONS |= HEX;
      else if (!strcmp(optarg, "rgb")) OPTIONS |= RGB;
      else if (!strcmp(optarg, "hsv")) OPTIONS |= HSV;
      else if (!strcmp(optarg, "hsl")) OPTIONS |= HSL;
      else {
  ...
  if ((OPTIONS & NEGATE) == NEGATE) color = negate(color);

  /* display the result*/
  displaycolor(color, OPTIONS);
...
color_t createhex(char *hex)
{
  ...
  if (strspn(hexvalue, "0123456789abcdefABCDEF") != strlen(hexvalue) || strlen(hexvalue) != 6)
    EXIT_ON_FAILURE("invalid hex format: %s", hex)
  int r, g, b;
...
color_t createhsl(char *hue, char *saturation, char *lightness)
{
  ...
  /* range validation */
  if (!(0 <= h && h < 360)) EXIT_ON_FAILURE("invalid hue value: %s\n", hue)
  if (!(0 <= s && s <= 100)) EXIT_ON_FAILURE("invalid saturation value: %s\n", saturation)
...
static color_t rgb2hsv(color_t rgb)
{
  ...
  float hue = fmod((hue =
    max == min ? 0.f :
    max == r ? 60.f * (fmod((g - b), 6.f) / (max - min)) :
...
color_t negate(color_t color)
{
  ...
  default:
    EXIT_ON_FAILURE("invalid negation\n")
  }
}
...
static color_t negatergb(color_t rgb)
{
  ...
  negated_rgb.values[0] = fmod(255 - negated_rgb.values[0], 256);
  negated_rgb.values[1] = fmod(255 - negated_rgb.values[1], 256);
  negated_rgb.values[2] = fmod(255 - negated_rgb.values[2], 256);
  return negated_rgb;
}
...
static color_t negatehsv(color_t hsv)
{
  ...
  negated_hsv.values[0] = fmod(negated_hsv.values[0] + 180.f, 360.f);
  return negated_hsv;
}
...
static color_t negatehsl(color_t hsl)
{
  ...
  negated_hsl.values[0] = fmod(negated_hsl.values[0] + 180.f, 360.f);
  return negated_hsl;
}

#define NEGATE_OP(neg, code) neg == 1 ? negate(code) : code

void displaycolor(color_t color, int options)
{
  if ((options & CONVERSION) == CONVERSION) {
    if ((options & HEX) == HEX) {
      switch (color.model) {
      case HEX: displayhex(color); break;
      case RGB: displayhex(rgb2hex(color)); break;
      case HSV: displayhex(hsv2hex(color)); break;
      case HSL: displayhex(hsl2hex(color)); break;
      }
    } else if ((options & RGB) == RGB) {
      switch (color.model) {
      case RGB: displayrgb(color); break;
      case HEX: displayrgb(hex2rgb(color)); break;
      case HSV: displayrgb(hsv2rgb(color)); break;
      case HSL: displayrgb(hsl2rgb(color)); break;
      }
    } else if ((options & HSV) == HSV) {
      switch (color.model) {
      case RGB: displayhsv(rgb2hsv(color)); break;
      case HEX: displayhsv(hex2hsv(color)); break;
      case HSV: displayhsv(color); break;
      case HSL: displayhsv(hsl2hsv(color)); break;
      }
    } else if ((options & HSL) == HSL) {
      switch (color.model) {
      case RGB: displayhsl(rgb2hsl(color)); break;
      case HEX: displayhsl(hex2hsl(color)); break;
      case HSV: displayhsl(hsv2hsl(color)); break;
